...
def get_data_from_csv(columns, countries=None, start=None, end=None):
    ...
    if countries is None:
        ...
        countries = df[df["date"] == end_date].sort_values("new_cases_per_million", ascending=False).head(6)
...
def plot_reported_cases_per_million(countries=None, start=None, end=None):
    ...
    # create dataframe
    cases_df = get_data_from_csv(columns, countries=countries, start=start, end=end)
    ...
    chart = (
        alt.Chart(cases_df, title="New cases per million")
        .mark_line()
        .encode(
            x=alt.X(
                "date:T",
